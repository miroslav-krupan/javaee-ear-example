...
Both WARs operate on the **same** backing EJB module — sharing `MemberRegistration`, `MemberRepository`, and the `Member` entity.
...
All business write operations flow through `MemberRegistration.register()`, which is a **Container-Managed Transaction (CMT)** `@Stateless` EJB. The default transaction attribute is `REQUIRED`, so every invocation runs inside a JTA transaction demarcated by WebLogic.
...
- WebLogic class-loading overrides ensure the application's logging jars take precedence over server-bundled ones.
...
- No `*.properties` config file is present in the repo; the `config.key` property falls back to the literal default `"Default value"`.

### Security
- No security constraints in `web.xml` or `weblogic.xml`. No `@RolesAllowed` / `@PermitAll` EJB annotations present. Security, if any, is enforced at the WebLogic server layer outside the application.
...
| EAR class-loader hierarchy | EAR-level isolation + WAR-level overrides |
